...
function MobileHeader({countryCode, title, isHome, openCart, openMenu}) {
  ...
  const styles = {
    ...
    container: `${
      isHome
        ? 'mobile-header bg-primary/80 dark:bg-contrast/60 text-contrast dark:text-primary shadow-darkHeader'
  ...
  return (
    <header role="banner" className={styles.container}>
      <div className="flex gap-12">
        ...
        <nav className="custom-menu flex gap-8">
          {navigationLinks.map((el, i) => (
            <Link key={el.class} key={`m-item-${i}`} to={el.link}>
              {el.title}
            </Link>
          ))}
            <Link to="#" target="#">
      ...
      {/* DISABLE SEARCH, ACCOUNT AND CART HEAD BUTTONS
